...
rm  "$LAB_MANUAL_PATH/manual-load-db.db" \
    "$LAB_ORCHESTRATED_PATH/airflow-load-db.db" \
    "$LAB_END_TO_END_PATH/basic-etl-load-db.db" \
    "$LAB_CHALLENGE_PATH/challenge-load-db.db"

# Manual Sqlite Database
cat create_table.sql | sqlite3 "$LAB_MANUAL_PATH/manual-load-db.db"
# <<EOF
...
cat create_table.sql | sqlite3 "$LAB_ORCHESTRATED_PATH/airflow-load-db.db" #<<EOF
#.read create_table.sql
#.quit
#EOF

# Basic Etl Sqlite Database
cat create_table.sql | sqlite3 "$LAB_END_TO_END_PATH/basic-etl-load-db.db" #<<EOF
#.read create_table.sql
#.quit
#EOF

# Challenge Sqlite Database
cat create_challenge_table.sql | sqlite3 "$LAB_CHALLENGE_PATH/challenge-load-db.db" #<<EOF
